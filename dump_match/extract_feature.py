...
if __name__ == "__main__":
  ...
  detector = ExtractSIFT(opt.num_kp)  #num_kp=2000 生成2000个关键点
  # get image lists 获取图像列表
  #input_path='../raw_data/yfcc100m/'
  #img_glob='*/*/images/*.jpg'
  search = os.path.join(opt.input_path, opt.img_glob)
  listing = glob.glob(search)

  for img_path in tqdm(listing):
    kp, desc = detector.run(img_path)  #用SIFT来提取特征点和描述符
    save_path = img_path+'.'+opt.suffix+'.hdf5'  #将提取出来的特征点和描述符保存为hdf5文件
